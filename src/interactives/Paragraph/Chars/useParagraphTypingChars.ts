...
interface IUseParagraphTypingChars {
...
export default function useParagraphTypingChars({
  refContent,
  delayTrigger,
  delayEnter,
  duration,
}: IUseParagraphTypingChars): {
  animationHide: () => void;
      ...
      if (splitter?.chars?.length)
        splitter?.chars?.forEach((char, key) => {
          gsap.to(char, {
            opacity: 1,
            scale: 1,
            delay: (delayIn || delay) + key / 30,
            ease: 'steps(1)',
            textShadow: '0 0 5px rgba(0,0,0,0.0)',
            duration: 1.2,
            // stagger: 0.015,
          });
        });
